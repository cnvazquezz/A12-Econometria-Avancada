#Aula 12 - Modelos  ARIMA ~ A12-Econometria Avançada
...
#Cria um vetor para o tempo em anos de 1994 até 2011 


                                    #Analise para o Emprego

plot(EMPREGO, type = "l")                            #Cria gráfico para o PIB #l de linha
emprego <- ts(EMPREGO, start = 1994, frequency = 1)  #Define como Série Temporal # 1 = anual
plot(emprego, main="Pessoa Empregadas no Brasil", # =Titulo
...
#Não é estacionacionaria, pq não tem um aqueda bruta nos dados

reglinEMP <- lm(EMPREGO ~ Anos)                      #Estimando a MQO  #Regressão linear simples do emprego em relação ao tempo
...
pdemprego <- diff(EMPREGO)                                #Calcula a primeira diferença da série de dados #Eliminia algumas diferenças
...
pdemprego1 <- diff(emprego)                                            #Calculando-se a primeira diferença #com lags = teste de dickfuller aumentado
